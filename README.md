...
> 旧「网易真方块包」路线（`src/index.ts` / `src/main.ts` / `ParserItemsAdderFullPack` / `EncoderBedrock*`，产网易格式 behavior+resource 双包）**已归档、不再维护**，当前 Skyblock 不使用。其用法、参数与常见问题见 git 历史。下面的 **Geyser 路线** 是当前唯一活跃的产出路径。

---

# Geyser 路线（IA → Geyser 自定义方块/物品/武器/粒子）

把 **ItemsAdder 的美术/配置作为唯一真相源**，自动生成 **Geyser `custom_mappings` + 基岩资源包**，并部署到 Geyser。
